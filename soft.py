...
def generate_random_mask(data, mask_ratio):
    """
    Generate a random mask for nodes instead of a semantic mask.
    ...
    # If train_mask exists, only mask nodes in the training set
    if hasattr(data, 'train_mask'):
        train_idx = data.train_mask.nonzero(as_tuple=False).view(-1)
        num_mask = min(num_mask, train_idx.size(0))
        # Randomly sample nodes from training set
        sampled = train_idx[torch.randperm(train_idx.size(0))[:num_mask]]
    else:
        # Randomly sample nodes from all nodes
        sampled = torch.randperm(num_nodes, device=data.x.device)[:num_mask]
    
    # Create mask
    mask = torch.zeros(num_nodes, dtype=torch.bool, device=data.x.device)
    mask[sampled] = True
    return mask
...
def soft_masking(x, mask, mask_token, beta=0.7):
    """
    Apply soft masking to node features.
    Combines original features with mask token using beta as weight.
    """
...
def pretrain_contrastive(model, data, pretrain_epochs=20, temp=0.5):
    ...
    for epoch in range(pretrain_epochs):
        ...
        # Generate two different random masks for creating two views
        mask1 = generate_random_mask(data, random.uniform(0.2, 0.4))
        mask2 = generate_random_mask(data, random.uniform(0.2, 0.4))
...
def train_model(model, data, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        # Generate random mask instead of semantic mask
        mask = generate_random_mask(data, random.uniform(0.2, 0.4))
        
        # Still use soft masking
...
def main():
    ...
    for config in dataset_configs:
        ...
        torch.save(model.state_dict(), f'{dataset_name.lower()}_model_soft.pt')
